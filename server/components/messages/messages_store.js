...
const {
  get_search_query,
  plugins: { Only_Model_Props, String_To_Regex },
} = require("../../store/utils");
...
async function update(query, udpate) {
  const updated_document = await Model.findOneAndUpdate(query, udpate, {
    new: true,
  });
  if (!updated_document)
    throw new Error(
      `Next query: ${JSON.stringify(query)} not found any document`
    );
...
async function updateText(document_id, text) {
  ...
  if (!updated_document)
    throw new Error(`Document Id: ${JSON.stringify(document_id)} not found`);
